  ...
  return (
    <section className="relative min-h-screen bg-gradient-to-br from-orange-500 to-orange-600 overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10">
        <div className="absolute top-10 left-10 text-8xl text-white">🌶️</div>
        <div className="absolute top-32 right-20 text-6xl text-white">🍛</div>
        <div className="absolute bottom-20 left-20 text-7xl text-white">🥘</div>
        <div className="absolute bottom-32 right-10 text-5xl text-white">
          ✨
        </div>
      </div>
      ...
      <div className="relative z-10 container mx-auto px-4 py-12 md:py-20 flex flex-col lg:flex-row items-center min-h-screen">
        ...
        >
          ...
          >
            Authentic <span className="text-white">Desi</span> Combos
            <br />
            <span className="text-orange-100">Delivered Fresh!</span>
          </motion.h1>

          <motion.p
            className="text-lg md:text-xl lg:text-2xl text-orange-100 mb-6 md:mb-8 max-w-2xl px-4 md:px-0"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.6 }}
          >
            From sizzling Chinese combos to royal Biryani party packs -
            experience the magic of home-cooked flavors, delivered straight to
            your doorstep! 🏠✨
          </motion.p>

          <motion.div
            className="flex flex-col sm:flex-row gap-3 md:gap-4 justify-center lg:justify-start px-4 md:px-0"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <Button
              size="lg"
              className="bg-white text-orange-500 hover:bg-gray-50 font-bold px-6 md:px-8 py-3 md:py-4 rounded-full shadow-2xl transform hover:scale-105 transition-all duration-300 w-full sm:w-auto"
              onClick={scrollToMenu}
            >
              <Utensils className="mr-2" size={18} />
              Order Now - ₹99 Onwards!
            </Button>
            <Button
              size="lg"
              color="light"
              className="bg-white/20 backdrop-blur-sm text-white border-white hover:bg-white/30 px-6 md:px-8 py-3 md:py-4 rounded-full w-full sm:w-auto"
              onClick={() => window.open("https://wa.me/8076507512", "_blank")}
            >
              📱 WhatsApp Order
            </Button>
          </motion.div>
